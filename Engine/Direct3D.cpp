...
HRESULT Direct3D::InitToonShader()
{
	...
	assert(pCompileVS != nullptr);
	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_TOON].pVertexShader_));
	if (FAILED(hr))
	...
	hr = pDevice_->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_TOON].pVertexLayout_));
	if (FAILED(hr))
	{
		//エラー処理
		MessageBox(nullptr, "頂点インプットレイアウトに失敗しました", "エラー", MB_OK);
...
HRESULT Direct3D::InitToonOutLineShader()
{
	using namespace Direct3D;
	HRESULT hr;
	...
	hr = pDevice_->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &(shaderBundle[SHADER_OUTLINE].pVertexLayout_));
	if (FAILED(hr))
	{
		//エラー処理
		MessageBox(nullptr, "頂点インプットレイアウトに失敗しました", "エラー", MB_OK);
